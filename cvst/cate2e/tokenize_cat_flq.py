...
cn_dev = dict()
with open("/data/lenam/topics/cat/cn_dev_flq.tsv","r") as f:
	for line in tqdm(f):
...
cn_test = dict()
with open("/data/lenam/topics/cat/cn_test_flq.tsv","r") as f:
	for line in tqdm(f):
		qid, context, query = line.strip().split("\t",2)
		topic, turn = qid.rsplit('_',1)
...
cn_train = dict()
with open("/data/lenam/topics/cat/cn_train_flq.tsv","r") as f:
	for line in tqdm(f):
		qid, context, query = line.strip().split("\t",2)
		topic, turn = qid.rsplit('_',1)
...
tc20 = dict()
with open("/data/lenam/topics/cat/tc20_flq.tsv","r") as f:
	for line in tqdm(f):
		qid, context, query = line.strip().split("\t",2)
		topic, turn = qid.rsplit('_',1)
...
tc21 = dict()
with open("/data/lenam/topics/cat/tc21_flq.tsv","r") as f:
	for line in tqdm(f):
		qid, context, query = line.strip().split("\t",2)
		topic, turn = qid.rsplit('_',1)
...
with open("/data/lenam/cvst/cate2e/cat_tokens_flq.pkl","wb") as f:
